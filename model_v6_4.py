...
CENTER_LOSS_WEIGHT = 0.25          # retained for reading V6.2 configs
OFFSET_LOSS_WEIGHT = 1.00          # retained for reading V6.2 configs
...
INNER_DISTANCE_LOSS_WEIGHT = 1.00
...
def build_all_targets(
    ...
    single inner-distance field.  is retained
    unused so a V6.2 checkpoint can still be read and compared against.
    """
...
def build_model_v6_2_instance(
    ...
    # One stack where V6.2 had two. The centre head (4 channels of sparse
...
def _validate_model_v5_transfer_source(source_model: Model) -> None:
    ...
    expected_outputs = {
        "semantic": (None, IMG_SIZE, IMG_SIZE, NUM_CLASSES),
        "inner_distance": (None, IMG_SIZE, IMG_SIZE, 1),
        "boundary": (None, IMG_SIZE, IMG_SIZE, 1),
    }
    for layer_name, expected_shape in expected_outputs.items():
        try:
...
def watershed_instances(
    ...
    * **No centre peaks.** Seeds come from the field's own high ground, so an
    ...
    A pixel joins the nearest core, exactly as `boundary_partition` restores
    ridge pixels -- one distance transform with DIST_LABEL_CCOMP, which labels
    by connected zero-component, so every core keeps one identity.
    """
    ...
    # Threshold relative to each connected foreground blob's own maximum, not
    # against an absolute 0.5.
    #
    # The target field peaks at exactly 1.0 per instance, but a prediction need
    # not: a small or thin object is the kind a model under-predicts, and under
    # an absolute threshold such an object yields no core at all. It does not
    # merely lose its own identity -- having no core, its pixels are handed to
    # the nearest *other* object's core, so it disappears into its neighbour.
    # Measured on the synthetic scenes, a mild blur made a 16 px disc beside a
    # 130 px rectangle vanish into it, 2 objects becoming 1.
    #
    # Relative thresholding removes the assumption that prediction magnitude is
    # calibrated, and changes nothing on a well-formed field, where every blob
    # maximum is 1.0 and this reduces exactly to the absolute rule.
    blob_count, blobs = cv2.connectedComponents(
        foreground.astype(np.uint8), connectivity=8
    )
    blob_peak = np.zeros(max(blob_count, 1), dtype=np.float32)
    np.maximum.at(blob_peak, blobs[foreground], field[foreground])
    # A blob whose prediction is flat zero has no core to find; leave its
    # threshold at 1.0 so nothing is selected rather than everything.
    blob_peak = np.where(blob_peak > 0.0, blob_peak, 1.0)

    cores = foreground & (field >= threshold * blob_peak[blobs])

    # Bridge one-pixel gaps in a core, the same conservative closing
    # `boundary_partition` applies to boundary ridges and for the same reason.
    # An annulus is the case that needs it: at head resolution a `circle` is a
    # ring roughly three pixels thick, so its core is a one-pixel-wide
    # centreline with no redundancy at all, and a single noisy pixel splits the
    # ring into arcs that are each large enough to clear the area floor. The
    # kernel stays at one pixel so it can only rejoin what noise separated,
    # never bridge two genuinely distinct cores, which sit half an object
    # apart.
    ...
    if cores.any() and int(minimum_core_area) > 1:
        # A core below this area is noise, not an object's inner half. This
        # has to be applied to the *cores*, before expansion: afterwards every
        # core has grown into a plausible-looking instance.
        count, labelled = cv2.connectedComponents(
            cores.astype(np.uint8), connectivity=8
        )
        if count > 1:
            areas = np.bincount(labelled.ravel(), minlength=count)
            keep = np.zeros(count, dtype=bool)
            keep[1:] = areas[1:] >= int(minimum_core_area)
            if keep[1:].any():
                cores = keep[labelled]
            # If every core is below the floor the object is genuinely tiny;
            # keep what was found rather than discarding the object entirely.

    if not cores.any():
        # Nothing rose above the core threshold anywhere -- a uniformly flat
        # prediction. Treating each connected foreground blob as one instance
        # is the honest degenerate answer: it under-segments touching objects
        # rather than inventing splits the field gives no evidence for.
        count, blobs = cv2.connectedComponents(
            foreground.astype(np.uint8), connectivity=8
        )
        if count <= 1:
            return labels
        return _relabel_by_area(blobs, minimum_instance_area)
    ...
    # Expand every core over the whole class foreground by nearest distance.
    distance_source = np.ones(foreground.shape, dtype=np.uint8)
    ...
    grouped = np.where(foreground, nearest_core, 0).astype(np.int32)
    return _relabel_by_area(grouped, minimum_instance_area)
...
def decode_instances(
    ...
    * centre peaks, and with them `CENTER_CONFIDENCE_THRESHOLD`, whose 0.10
    ...
    The fourth return value is still the per-instance detection score, but it
    now comes from the peak predicted inner distance rather than a centre peak.
    It plays the same role and for the same reason: mean semantic probability
    saturates near 1.0 for real objects and fragments alike, so ranking needs a
    quantity that actually varies. A confident, well-formed object has a high
    predicted distance at its core; a sliver of rim does not.
    ...
    for class_id in range(1, NUM_CLASSES):
        ...
        for local_id in np.unique(grouped):
            ...
            # Peak predicted distance inside the candidate: the replacement
            # for the centre-peak score, and the only per-instance quantity
            # here that separates a solid object from a rim fragment.
            score = float(np.max(distance_full[candidate]))
...
def decode_instances_centre_offset(
    ...
    """
    Decode instances the V6.2 way, from centre peaks and an offset field.

    Retained unused so a V6.2 checkpoint can still be decoded and compared
    against; `decode_instances` below is what V6.4 runs. Not deleted because
...
def output_probabilities(
    ...
    # The V6.4 inner-distance head uses sigmoid, so its output is
    ...
    *,
    ...
    configuration = {
        ...
        "losses": {
            ...
            "inner_distance": {"name": "MaskedInnerDistanceLoss"},
            "offset": {
...
def run_selftest() -> None:
    ...
    # the dihedral stream is broken when it is not.
    global USE_COPY_PASTE_AUGMENTATION
